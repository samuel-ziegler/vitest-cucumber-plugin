...
const filterStars = () => _.filter((v: string) => v !== '*');
const cleanKeywords = (keywords: string[]): string[] => filterStars()(trimKeywords()(keywords));

const createState: (languageKeywords: Record<string, string | string[]>) => object = (languageKeywords) => {
    let state = base;
    ...
    const repeatStepReducer = (keywords: string[], repeatStepKey: string): string[] => {
    ...
    const otherKeyReducer = (acc: object, key: string): object => {
...
const gherkinLexerConfig: (options: VitestCucumberPluginConfig) => void = (options) => {
    ...
    const languageKeyReducer = (acc: object, languageKey: string) =>
        _.set(languageKey, createState(gherkinLanguages[languageKey]), acc);
